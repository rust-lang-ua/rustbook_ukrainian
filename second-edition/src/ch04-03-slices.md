...
параметрі 's' елемент за елементом і перевірити, чи не пробіл це, ми створюємо ітератор по символах за допомогою методу `chars`:
...
що `chars` - метод, що повертає кожен елемент в колекції. Потім обгортаємо кожен елемент колекції за допомогою методу `enumerate` у нумерований кортеж:

```rust,ignore
for (i, &item) in chars.enumerate() {
```

Перший елемент кортежу - індекс, починаючи з 0, а другий - значення елементу. Це
трохи зручніше, ніж обчислювати індекс самостійно.
...
складається з індексу `i` і елементу `item` в кортежі.

Ми шукаємо символ пробілу. Коли знаходимо його, повертаємо його індекс. Якщо 
цього не сталося, повертаємо довжину стрічки за допомогою методу `s.len()`:
