...
string to the `u32`, which can only contain numerical data. The user must press <span class="keystroke">enter</span> to satisfy `read_line` and input their
...
method does (discussed earlier in “Handling Potential Failure with <!-- ignore-->). We’ll treat
