...
The scope in which the variable `s` is valid is the same as any function parameter’s scope, but the value pointed to by the reference is not dropped when `s` stops being used, because `s` doesn’t have ownership. Коли функції мають параметри - посилання замість значень, нам не треба повертати значення, щоб повернути володіння, бо ми й не мали володіння.
...
So, what happens if we try to modify something we’re borrowing? Спробуйте запустити код з Блоку коду 4-6. Обережно, спойлер: він не працює!
...
First we change `s` to be `mut`. Потім ми створюємо мутабельне посилання за допомогою `&mut
...
Це обмеження, що забороняє кілька мутабельних посилань на одні й ті самі дані в один час, дозволяє їх змінювати, але під пильним контролем. It’s something that new Rustaceans struggle with because most languages let you mutate whenever you’d like. Перевага цього обмеження в тому, що Rust може запобігти гонитві даних під час компіляції. *Гонитва даних* подібна до стану гонитви й стається, коли мають місце такі три умови:
...
Області видимості немутабельних посилань `r1` і `r2` завершуються після `println!`, де вони востаннє використані, тобто перед створенням мутабельного посилання `r3`. These scopes don’t overlap, so this code is allowed: the compiler can tell that the reference is no longer being used at a point before the end of the scope.
...
In languages with pointers, it’s easy to erroneously create a *dangling pointer*—a pointer that references a location in memory that may have been given to someone else—by freeing some memory while preserving a pointer to that memory. У Rust натомість компілятор гарантує, що посилання ніколи не стануть висячими: якщо ви маєте посилання на певні дані, компілятор пересвідчиться, що дані не вийдуть з області видимості раніше за посилання на ці дані.
