...
<!-- ignore -->(prelude). Якщо типу, який ви хочете використати, нема у 
...
String. [`String`][string]<!-- ignore --> - це тип стрічки, що надається 
...
Далі ми додаємо всередині ще два рядки. Функція rand::thread_rng дасть нам 
...
інструкції `use`. Ми скористалися виразом [`match`][match]<!-- ignore -->, щоб 
...
(#handling-potential-failure-with-the-result-type)<!-- ignore -->). Ми 
